...
if (empty($faltantes)) {
    ...
    if (!$conexion) {
        ...
        //Consulta viajes
        $consulta = "SELECT *  FROM viajes where operador = '$operador' and estatus not in ('Pendiente','Finalizado')";
            ...
            if (count($dataViaje) > 1) {
                ...
                switch ($dataViaje["estatus"]) {
                    ...
                    case "En proceso":
                        ...
                        if (empty($dataTramos)) {
                            ...
                            $payload = [
                                ...
                                "fechaCarga" => $dataViaje["fecha_carga"],
                                "unidad" => $dataViaje["unidad"],
                                "observacionesViaje" => $dataViaje["ruta"],
                    ...
                    case "En trayecto":
                        ...
                        $id = $dataViaje["id"];
                        //Consulta tramos
                        $consultaTramos = "SELECT id, tramo, fecha, entrega, distancia, observaciones, cajas, embarque, estatus FROM tramos where idViaje = $id";
                        $tramos = mysqli_query($conexion, $consultaTramos);
                        while ($row = $tramos->fetch_array(MYSQLI_ASSOC)) {
                            $dataTramos[] = $row;
                        }
                        if (empty($dataTramos)) {
                            ...
                            if (count($dataTramos) > 1) {
                                $payload = [
                                    ...
                                    "numeroTramo" => $dataTramos[0]["ruta"] . "-" . count($dataTramos) + 1,
                                    "destinoTramo" => $dataTramos[0]["entrega"],
