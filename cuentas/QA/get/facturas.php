...
if (empty($faltantes)) {
    ...
    if (!$conexion) {
        ...
        //Consulta viajes
        $consulta = "SELECT id,cliente , viaje, precio,sum(precio+gEstadias+gManiobras+Adicionales+desviacion) monto, sum(gEstadias+gManiobras+Adicionales+desviacion) Adic, factura,fecFacturacion, montoPagado FROM operaciones where status ='27' and factura <> 'N/A' and year(fecha)='2020' group by viaje ORDER BY fecFacturacion  DESC, viaje DESC";
        if ($id !== null) {
            $consulta = "SELECT id,cliente ,viaje,sum(precio+gEstadias+gManiobras+Adicionales+desviacion) monto,sum(gEstadias+gManiobras+Adicionales+desviacion) Adic,factura,fecFacturacion, montoPagado FROM operaciones where status ='27' and factura <> 'N/A' and year(fecha)='2020'  and id=$id ";
        }

        $facturas = mysqli_query($conexion, $consulta);
        while ($row = $facturas->fetch_array(MYSQLI_ASSOC)) {
            if ($row['monto'] > 0) {
                $row['monto'] = round($row['monto'] + ($row['monto'] * .16) - ($row['precio'] * .04) - ($row['Adic'] * .06), 2);
