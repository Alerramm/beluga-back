...
if (empty($faltantes)) {
    ...
    if (!$conexion) {
        ...
        if ($idApp != '') {
            ...
            foreach ($datos["rutas"] as &$tramo) {
                $destino =  $tramo["ciudad"];
                $idTramo = $tramo["id"];
                $waypoints  = $tramo["polyline"];
                $payload["tramo " . $tramo["tramo"]] = update($conexion, "UPDATE tramos SET destino = '$destino', waypoints = '$waypoints' WHERE id = $idTramo");
            }
            $payload["viaje"] = update($conexion, "UPDATE viajes SET estatus = 'Pendiente', estatus_app = 'Pendiente' WHERE id = $idApp");
            respuesta(200, 200, "Total record", $payload);
        } else {
            if ($cliente == "0") {
                        ...
                        if ($conexion->query($insertV) === true) {
                            ...
                            foreach ($datos["rutas"] as &$valor) {
                                ...
                                if ($conexion->query($insertT) === true) {
                                    ...
                                    foreach ($valor["desgloze_casetas"] as &$casetas) {
                                        $name = $casetas["name"];
                                        $costoC = $casetas["cashCost"];
                                        $insertCasetas = "INSERT INTO casetas(idTramo, nombre, precio) VALUES ($last_id_trip,'$name','$costoC')";
                                        if ($conexion->query($insertCasetas) === true) {
                                            $last_id_caseta = $conexion->insert_id;
                                            $payloadGastosInsert[] = ["casetasInsert" => " Exito New Travel record created successfully " . $last_id_caseta];
                                        } else {

                                            $payloadGastosInsert[] = ["casetasInsert" => " Error al insertar caseta " . $insertCasetas];
                                        }
                                    }
